...
(() => {
  ...
  // src/model/rightsizing.ts
  var GIB_PER_CORE = 4;
  var GIB = 2 ** 30;
  function sizingRows(c) {
    const history2 = c.usage.source === "prometheus";
    return sized(c).map((s) => {
      const cpu = {
      ...
      cpu.verdict = cpuVerdict(cpu.request, cpu.used, history2);
      const memory = {
      ...
      memory.verdict = memoryVerdict(memory.request, memory.limit, memory.used, history2);
      const requests = {};
      const limits = {};
      ...
      const from = (n) => Number.isFinite(n) ? n : 0;
      const change = {
        cpu: requests.cpu !== void 0 ? (requests.cpu - from(cpu.request)) * pods : 0,
        memory: requests.memory !== void 0 ? (requests.memory - from(memory.request)) * pods : 0,
        memoryLimit: limits.memory !== void 0 && Number.isFinite(memory.limit) ? (limits.memory - memory.limit) * pods : 0,
        pods
      };
      ...
      return {
        ...
        change,
        impact,
        fix: history2 && anything ? resourceFix(s, { requests, limits }) : void 0
      };
  ...
  function sourceNotice(u, openUrl) {
  ...
  // src/pages/rightsizing.ts
  var REFRESH_EVERY = 6e4;
  var USAGE_EVERY = 5 * 6e4;
  var state = { ctx: null, snap: null, usage: null, usageAt: 0, rows: [], system: false, query: "", onlyTips: true };
  function fail(err) {
  ...
  function hasTip(row) {
    const off = (s) => s.verdict !== "ok" && (s.verdict !== "unknown" || !Number.isFinite(s.request) && Number.isFinite(s.used));
  ...
  function cell(sizing, format, quantity) {
    const td = el("td", "num");
    const verdict = !Number.isFinite(sizing.request) && Number.isFinite(sizing.used) ? { text: "no request", tone: "warn" } : VERDICT[sizing.verdict];
  ...
  function renderTable() {
    ...
    const rows = visible();
    const history2 = u.source === "prometheus";
    box.appendChild(
      el(
        "p",
        "faint small",
        `${plural(rows.length, "container")} shown of ${state.rows.length} with readings, biggest change first (a core weighs as much as ${GIB_PER_CORE} GiB). ` + (history2 ? "Usage is the last day: CPU at its 95th percentile, memory at its peak." : "Usage is one reading from metrics-server, so there is no Apply: a request is not sized on one moment.")
      )
    );
    if (!rows.length) {
  ...
  var APP_KINDS = { Deployment: "deployments", StatefulSet: "statefulsets", DaemonSet: "daemonsets", CronJob: "cronjobs", Job: "jobs" };
  sdk.ready().then(async (ctx) => {
    state.ctx = ctx;
    const hash = readHash();
    state.query = hash.q ?? "";
    const view = durable ? await recall("rightsizing", {}) : {};
    state.onlyTips = typeof view.onlyTips === "boolean" ? view.onlyTips : hash.all !== "1";
    state.system = typeof view.system === "boolean" ? view.system : hash.sys ? hash.sys === "1" : (await loadSaved()).settings.system;
